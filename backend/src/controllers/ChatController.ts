...
export const initiateChatSession = async (req: Request, res: any) => {
  ...
  try {
    const existingSession = await pool.query(
      "SELECT id FROM chat_sessions WHERE customer_id = $1 AND agent_id = $2 AND status = 'active'",
      [customerId, agentId],
    )
...
export const listAgents = async (req: Request, res: any) => {
  try {
    const result = await pool.query("SELECT id, name FROM users WHERE role = $1", ["agent"])
